...
export function ArcFace({
  ...
  return (
    ...
    >
      ...
      <rect x="33" y="22" width="11" height="32" rx="5.5" fill="#FFFFFF" />
      {/* Right eye — vertical pill */}
      <rect x="56" y="22" width="11" height="32" rx="5.5" fill="#FFFFFF" />
      {/* Mouth */}
...
function Mouth({ state, visemeIdx }: { state: ArcFaceState; visemeIdx: number }) {
  if (state === "speaking") {
    ...
    if (v === "E") {
      return <ellipse cx="50" cy="76" rx="10" ry="3.4" fill="#FFFFFF" />
    }
    if (v === "O") {
      return <ellipse cx="50" cy="76" rx="6.2" ry="6.4" fill="#FFFFFF" />
    }
    // "A" — wide & tall (loudest viseme)
    return <ellipse cx="50" cy="76.5" rx="9.6" ry="8.4" fill="#FFFFFF" />
  }

  if (state === "thinking") {
    return <ellipse cx="50" cy="76" rx="3.2" ry="3.2" fill="#FFFFFF" />
  }
