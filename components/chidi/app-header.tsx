...
import { ArrowLeft, Calendar as CalendarIcon, ChevronRight, StickyNote } from "lucide-react"
...
interface AppHeaderProps {
  ...
  /**
   * When set, renders a back button + page title in place of the identity
   * block (the Spaces switcher is reachable from Settings). Used on every
   * secondary route (Settings, Notebook, Customers, Board, Calendar) so the
   * merchant always has a one-tap exit to where they came from.
   *
   * `back.label` shows next to the arrow as the page title; `back.fallback`
   * is where router.back() falls through to when there's no history (deep
   * link landing or PWA cold-start).
   */
  back?: {
    label: string
    fallback: string
  ...
  return (
    <header className="sticky top-0 z-40 bg-[var(--background)] border-b border-[var(--chidi-border-default)] safe-area-top">
      <div className="flex items-center justify-between h-14 px-3 max-w-lg mx-auto gap-2">
        {back ? (
          // Back-mode left block — used on every secondary/Library route. The
          // identity block (Spaces switcher) is reachable from Settings, so we
          // sacrifice it here to give the merchant a real one-tap escape hatch.
          // The arrow + page-name reads like iOS / standard mobile chrome.
          <button
            type="button"
            onClick={handleBack}
            aria-label={`Back from ${back.label}`}
            className="inline-flex items-center gap-1.5 min-h-[44px] min-w-[44px] max-w-[60%] min-w-0 px-2 -mx-1 rounded-lg hover:bg-[var(--chidi-surface)] active:scale-[0.98] transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--chidi-win)]/40 text-[var(--chidi-text-primary)]"
          >
            <ArrowLeft className="w-5 h-5 flex-shrink-0" strokeWidth={2} />
            <span className="text-[14px] font-medium truncate font-chidi-voice">
              {back.label}
            </span>
          </button>
        ) : (
          // Default identity block — opens the Spaces switcher in a bottom Sheet.
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0">
          {/* Calendar Peek + Notes — only on the dashboard root. On secondary
              routes (back-mode), they collide with the back button + the
              destination page already gives access to its own utilities, so
              we hide them and keep just notifications. */}
          {!back && (
            <>
